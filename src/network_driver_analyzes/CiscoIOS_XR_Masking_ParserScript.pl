...
sub maskPasswords
{
	...
	foreach $line (@lines)
	{
		...
		# IOS-XR SNMPv3
		# snmp-server user imandi12 testv3group v3 auth sha encrypted 05020B0E2F4847061700
		$line =~ s/(snmp-server [\S ]+?auth \S+ (clear|encrypted)? ?)\S+/$1xxx/g;
		$line =~ s/(priv des56 (clear|encrypted)? ?)\S+/$1xxx/g;
		$line =~ s/(snmp-server user ).*/$1xxx/g;
		...
		# radius server key
		$line =~ s/radius-server key \S+/radius-server key xxx/g;
		$line =~ s/(radius-server host \S+ auth-port \d+ acct-port \d+ key \d+) \S+/$1 xxx/g;
		# crypto keys
