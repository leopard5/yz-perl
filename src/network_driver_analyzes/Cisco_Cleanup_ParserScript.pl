...
sub cleanupFileSystem
{
	...
	while($cleandata =~ /dir (\S+:(\/\S+\/)?|\/all)/gc)
	{
		$ios_workaround=0;
		$slot = $1;
		...
		# If there's a prompt in there, then get rid of it and anything afterward
		($subData,$junk) = split(/\n\S+\# ?\n/,$subData,2);
