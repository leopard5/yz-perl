...
sub cleanupConfiguration
{
	...
	# Extract the config block using the leading and ending line... whatever the leading line is
	$config = $1 if $config =~/.*?(! ACNS version.*?End of ACNS configuration)/si;
	$config = $1 if $config =~/.*?(show running-config.*?End of ACNS configuration)/si;
	$config = $1 if $config =~/.*?(show startup-config.*?End of ACNS configuration)/si;

	# Remove MOREs
	$config = removeMores($config) . "\n";
	
	# Remove the leading line... which ever of the three it is
	$config =~ s/^\s*! ACNS version.*\n//;
	$config =~ s/^\s*show running-config\n//;
	$config =~ s/^\s*show startup-config\n//;
...
sub cleanupRouting
{
	...
	if($cleandata !~ /(Invalid input|BGP not active)/gc)
	{
...
sub cleanupFileSystem
{
    ...
    $start = $end = -1;    
    if($cleandata =~ /(#? *)(dir (\S+:|\/all))/gc )     
    {   
        $start = pos($cleandata) - length($2);      
    }   
    $cleandata = substr($cleandata, $start);    
     
    # Will now have a set of values such as:    
    # dir bootflash:     
    ...
    # lab-5500-r1#dir disk0:   
    # %Error opening disk0:/ (Invalid DOS media or no media in slot)
	# lab-5500-r1#dir disk1:
	while($cleandata =~ /dir (\S+:|\/all)/gc)
	{
		$ios_workaround=0;
		$slot = $1;
		...
		$start = $end = -1;
		$start = pos($cleandata);
		if($cleandata =~ /([\S ]*dir (\S+:|\/all))/gc)
		{
			$end = pos($cleandata) - length($1);
			pos($cleandata) = $end;
		}
		if($start > 0)
		{
			if($end > -1)
			{
				$subData = substr($cleandata, $start, $end - $start);
			}
			else
			{
				$subData = substr($cleandata, $start);
			}
		}
		# If slot is /all and there is another flash: entry, then skip this one.
		#if($slot eq "/all"){
		#	if($cleandata =~ /dir flash:/){
                # Sometimes, the second one can have no file information while the first one has data.
                
        #        print "Hello world $slot\n\n\n";
		#		next;
		#	}
		#}
		# Try to work around an oddity with IOS where the filesystem is not viewable
		# unless /all is used. 
		if($slot eq "/all"){
			$ios_workaround = 1;
			if($subData =~ /Directory of (\S+:)/)
			{
				$slot = $1;
				# Does cleandata still contain $slot?
				if($cleandata =~ /dir\s+$slot/){
					$ios_workaround = 0;
				}
			}
            if($subData =~ /(No such file|No files in directory)/ && $cleandata =~ /dir flash:/){
                # This command returned no results AND another entry for flash: exists; use that one
                next;
            }
		}
		# Ignore sections where errors occur
		if($subData !~ /(\%Error opening|No space information available)/)
		{
			$slot = "undefined";
			if($subData =~ /Directory of (\S+):/)
			{
				...
				if($ios_workaround == 1)
				{
					$subData =~ s/Directory of (\S+):/Directory of $1/;
				}
			}

		    # Sometimes, aliased sections may exist -- such as flash: -> slot0:
			if(! defined $SLOTS{$slot} )
			{
				$constructed_data .= $subData . "\n";
			}
			$SLOTS{$slot} = $slot; # Simply define
		}
	}

	$cleandata = $constructed_data;
	return $cleandata;
